...
def update(drone):
    ...
    err = heading_error(TARGET_HEADING, yaw_attitude)
    yaw_rate = uav_utils.clamp(err * KP_YAW, -MAX_YAW, MAX_YAW)
    ...
    if _hold >= HOLD_TIME:
        drone.flight.stop()
        print("Finished Homing on Heading")
        print(f"Target: {TARGET_HEADING} | Current: {yaw_attitude:.3f}")
        _done = True
